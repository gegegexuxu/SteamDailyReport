...
`STEAM_ID` 支持逗号分隔多个 SteamID64（本地 `.env` 与 GitHub Secret 同理），每个账号独立统计、各发一张卡片到同一个群：

- 新加入的账号自动初始化基线并推送「战报存档已创建」卡片，次日开始出报；
- 各账号的统计窗口、成就基线、「已发送」标记完全隔离，单个账号拉取或发送失败不影响其他账号；
- 从列表移除账号只是停止更新它的数据，历史快照原样保留；重新加回会补发一份跨天合并的战报（卡片会注明统计区间）；
- 查询别人的账号需要对方在 Steam 隐私设置里把「游戏详情」设为公开。

> 旧版本（v3 及更早）的单账号状态文件会在下次运行时自动迁移为按账号分桶的新格式，旧数据归属当前配置的第一个账号，无需手动处理。
