...
import { fatalError, postJson } from "./notifiers/http.js";
  ...
  try {
    await postJson(webhook, body, {
      parseResult: (res, data) => {
        ...
        if (res.ok && code === 0) return null;
        const hint = data.msg ?? data.StatusMessage ?? "";
        // 签名错误重试也不会成功，直接失败并给出修复提示
        if (hint.includes("sign") || hint.includes("签名")) {
          return fatalError(
            `飞书返回错误 code=${code ?? res.status}: ${hint}` +
              "（请检查 NOTIFY_SECRET 是否与机器人签名密钥一致）",
          );
        }
        return new Error(`飞书返回错误 code=${code ?? res.status}${hint ? `: ${hint}` : ""}`);
      },
    });
  } catch (err) {
    throw new Error(`飞书消息发送失败: ${err.message}`);
  }
